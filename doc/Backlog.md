...
| yukkie/AgentVillage#609 | tech-debt | （なし） | - | refactor(frontend): remove remaining duplicate/dead selectors in SpectatorScreen.module.css | #604 の続き。表外に残る子孫形/standalone 重複・デッドルール・`.phaseNav`/`.filt` 同名二重定義・`styles.coAgent` 未定義参照を PR #607 の判定手順で一掃。`.action` 配下（#608 の対象）は触らない |
