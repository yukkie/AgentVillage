...
## 7. 開発ツール層 — 型系譜（type-lineage）の抽出層／解釈層の分離

中心データ型（例: `LogEvent`）の producer / consumer / persist / transform を機械的に全列挙する
開発支援ツール `type-lineage`（skills repo `~/.claude/skills/type-lineage/`、Issue #424）を運用する
にあたり、その出力を扱う **2つのレイヤーを明確に分離**する。両者は寿命（揮発 vs 永続）と責務
（客観的抽出 vs 主観的解釈）が異なるため、混在させると陳腐化した生データが知見として残ってしまう。

### 7.1 抽出層（揮発的）— Extraction layer

- **中身**: AST 由来の生グラフ（型の隣接リスト JSON と、その派生ビューの Mermaid 図）。
- **性質**: **揮発**。ソースコードが変わるたびに陳腐化するため**永続保存しない**。必要時に毎回再生成する。
- **置き場**: **解析対象リポジトリのルート相対** `.tmp/type-lineage/<型名>.json`（gitignore 下、
  `.gitignore` に `.tmp/type-lineage/` を登録済み）＋ **stdout** の二重シンク。**commit しない**。
- **スキーマ**: 言語中立の隣接リスト。エッジは `source`(python/js) と `confidence`(typed/heuristic)
  の**独立2軸**を持つ（詳細は `type-lineage/SKILL.md`）。現状 Python AST のみ。JS 経路は
  `meta.unreached.js` に「未到達」と明示する（拡張は #508）。
- **出力コントラクト（バトンパス）**: 「安定スキーマ＋発見可能な置き場」を固定することで、
  `idd` 事前調査（producer/consumer の全列挙）と #425（スメル評価）が**同一の契約**で読める。

### 7.2 解釈層（永続的）— Interpretation layer

- **中身**: 抽出層の生グラフを人が/AI が読んで得た**知見・評価**（どの transform が淀みか、
  どの consumer が壊れやすいか、スキーマ再設計の判断材料など）。抽出層と違い**寿命が長く**、
  設計判断の根拠として蓄積する価値がある。
- **置き場**: **本リポジトリの `doc/` 配下**（永続・レビュー可能・git 管理下）。
- **本 Issue（#424）でのスコープ**: ここでは**置き場の予約（この節の定義）のみ**を行う。
  解釈層の**中身の生成・評価軸の定義は #425（スメル評価スキル）の担当**であり、本 Issue では作らない。
