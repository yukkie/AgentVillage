...
#### 7.4.1 フォントサイズ使用実態の調査と将来トークン化案（#562）

`tokens.css` には色・フォントファミリー（`--serif`/`--sans`/`--mono`）・角丸のトークンはあるが、`font-size`
のトークンは存在しない。上の「サイズの相対ルール」（#530）は **将来の例外で規約が硬くなることを避けるため、
数値対応表を意図的に定めていない**。本サブセクションはその相対ルールを上書きするものではなく、**将来トークン化を
判断するための前提情報（現状どのサイズがどこで使われているか）を一覧化し、粒度案を提示する調査記録**である。
トークン定義・stylelint ルール追加・既存コードの置き換えは本記録のスコープ外であり、別 Issue で扱う。

> **位置づけ**: 「サイズの相対ルール」は *実装者の日常判断*（同一行内で揃える等）を担い、トークン化は *将来の構造化*
> である。後者が実れば前者の一部（サイズの単一ソース化）を機械的に担保できる。両者は矛盾せず棲み分ける。

##### 現状の `font-size` 使用実態（AC-1）

`frontend/src/` 配下の全 `*.module.css`（`tokens.css` を除く）と `App.module.css` から `font-size` 指定を
全数抽出した（調査時点）。サイズ値ごとの出現数と使用文脈は以下のとおり。

| px 値 | 出現数 | 代表セレクタ（ファイル） | 使用文脈 |
|---|---|---|---|
| `9px` | 1 | `.md .role`（Avatar） | アバター内の役職刻印（md サイズ） |
| `10px` | 31 | `.ts` / `.turn` / `.conf` / `.sysTs`（FeedCard）, `.when` / `.coMeta` / `.count` / `.deathline`（Spectator）, `.xs .mono`（Avatar）ほか多数 | タイムスタンプ・turn 番号・mono 系メタ・件数 count・薄字ラベル（**最多**） |
| `10.5px` | 1 | `.thinkBadge` 系（FeedCard） | 思考ログ系の小ラベル |
| `11px` | 21 | `RoleTag` ルート, `.coRole` / `.stat`（Spectator）, `.rule` / `.endedTag` / `.community`（GameList）, `.lockBadge` / `.strategyBadge`（FeedCard）ほか | 役職タグ・CO ラベル・サブラベル・ルール名・ロック/戦略バッジ |
| `11.5px` | 2 | `.action`（Spectator） | 夜行動タイムライン行 |
| `12px` | 19 | フォーム/カード本文系（GameList 多数）, `.coRow`（Spectator）, FeedCard 本文系, TopBar ボタン | カード本文・フォームラベル・操作系テキスト |
| `12.5px` | 1 | AgentDetail 本文系 | エージェント詳細の本文 |
| `13px` | 8 | `.sysText`（FeedCard）, GameList/AgentDetail/TopBar 見出し小・本文 | 本文・システム行テキスト・見出し小 |
| `13.5px` | 1 | `.spText`（FeedCard） | 発言カード本文 |
| `14px` | 11 | `.name` / `.spHead .name`（FeedCard）, AgentDetail 見出し系, App/TopBar 本文大, `.row`（AgentRosterRow）, `.sm .mono`（Avatar） | エージェント名・見出し系・本文大 |
| `15px` | 1 | GameList 見出し | 見出し |
| `16px` | 1 | ThreePaneLayout | 折りたたみラベル等 |
| `17px` | 1 | TopBar ロゴ | ブランドロゴ |
| `18px` | 1 | Spectator 見出し | ペイン見出し |
| `24px` | 1 | AgentDetail 大見出し | Hero 名 |
| `26px` | 1 | `.md .mono`（Avatar） | アバターイニシャル（md） |
| `32px` | 1 | AgentDetail Hero アバター | Hero アバター内テキスト |
| `clamp(28px, 6vw, 48px)` | 1 | App | 特大見出し（レスポンシブ） |

**分布の所見**:
- 全体の約 9 割が `10px`〜`14px` の「本文・メタ」帯に集中する。とくに `10px`（31）・`11px`（21）・`12px`（19）の
  3 値が骨格を成す。
- 半端値が 4 つ存在する（`10.5` / `11.5` / `12.5` / `13.5`、各 1〜2 箇所）。いずれも近接する整数値と用途が重なり、
  意図的な区別というより場当たりの結果と見られる。
- `15px` 以上の大きい値は **すべて単発**（見出し・ロゴ・アバター内テキスト）で、段階的スケールというより
  個別要素ごとの調整値。

##### フォントサイズトークン化の粒度案（AC-2）

上の実態から、**本文・メタ帯を 5 段スケールに畳み込む**案を提示する。半端値は最寄り段へ吸収し、
大見出し系はスケールとは別系列または個別維持とする。**これは調査に基づく提案であり、確定値ではない。**
実際のトークン名・px 値・段数は実装 Issue で再検討する。

| トークン名（案） | px（案） | 統合元の生値 | 主な使用文脈 |
|---|---|---|---|
| `--fs-xs` | 10px | 9, 10, 10.5 | タイムスタンプ・turn 番号・mono 系メタ・件数・薄字ラベル |
| `--fs-sm` | 11px | 11, 11.5 | 役職/CO タグ・stat・ルール名・サブラベル |
| `--fs-base` | 12px | 12, 12.5 | カード本文・フォームラベル・操作系テキスト |
| `--fs-md` | 13px | 13, 13.5 | 本文・システム行テキスト・発言カード本文 |
| `--fs-lg` | 14px | 14, 15 | エージェント名・見出し小・本文大 |

**スケール外（別系列または個別維持の論点）**:
- `16px` / `17px` / `18px` / `24px` / `32px` / `clamp(28px, 6vw, 48px)` は単発の見出し・ロゴ・アバター内テキスト。
  本文スケールに無理に畳み込むと意味が崩れるため、`--fs-h1`〜のような **見出し別系列**を立てるか、
  装飾的な単発値として**トークン化せず個別維持**するかを実装 Issue で判断する。
- 半端値（`.5px`）の吸収は視覚差が小さい前提だが、`.spText`（発言本文 13.5px）など本文の可読性に直結する箇所は
  吸収先（13 か 14 か）を実装時に実機確認する。

##### スコープ外（本記録で行わないこと）

- `tokens.css` への `--fs-*` トークン定義
- stylelint（§7.5）への `font-size` 検出ルール追加
- 既存 CSS の生値 → トークン置き換え

これらは AC-2 の案を土台に別 Issue で実装する。
