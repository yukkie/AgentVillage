...
データソース: `game_stats.json` / `agents/*.json` / `spectator_log.jsonl` / `frontend/src/config/agents.json`（§8.3 参照）。`stub/agentDetail.js` は #547 で削除済み。blurb（1行プロフィール）は `frontend/src/config/agents.json` の `blurb` フィールドを静的 import で取得（#519、#628 で fetch から import に変更）。
...
### 7.6 CSS ルールの到達性棚卸し（#620）

**到達性（そのセレクタが実際に DOM に一致するか）は stylelint の検出対象外**。§7.5 の design lint はトークン使用規約（生値ハードコード）だけを見ており、宣言されたルールが実際に使われているかは別の関心事のため、以下の手段で別途棚卸しする。

**静的到達性監査スクリプト（`npm run audit:css`）**

`frontend/scripts/cssDeadRules.mjs` — `src/**/*.module.css` の各セレクタについて、対応する production JS/JSX に consumer が存在するかを静的に検査する。セレクタ単位・モジュール単位スコープ（別名 import 対応）・外部依存なし・実行1秒未満。`npm run audit:css` で実行する。CI には搭載しておらず、棚卸し時に手動で叩く運用。

検出できるのは D1（宣言はあるが consumer が無い＝静的デッド候補）のみ。D2（子孫セレクタ等の構造上到達しない）・D3（分岐や prop 未到達で実行時に到達しない）は検出できないため、候補は出発点として扱い、削除前に必ず consumer を実読して判定すること（実例は #620 PR 参照）。

**ランタイム到達率を見たいときの手順と落とし穴**

`vitest.config.js` の `css: true`（#619）により、テスト実行中に CSS Modules が実解決される。これを使い、全テストスイートを走らせて各 CSS ルールが一度でも実 DOM に一致したかを計測すると、静的監査より広い「実際にテストで描画されたか」という粒度の情報が得られる（#620 で実施）。ただし恒常的な仕組みとしては採用していない（単発の棚卸し用、使い捨てのハーネスをコミットしない）:

- **`setupFiles` で DOM 挿入時にスイープする方式が必須**。`MutationObserver` で `childList`/`subtree`/`attributes` を監視し、DOM 変更のたびに読み込み済みスタイルシートの各ルールを `document.querySelector` で照合する
- **`afterEach` だけに頼ってはいけない**: React Testing Library の `cleanup`（各テスト末尾で自動実行）でDOMがアンマウントされた*後*に評価すると、そのテストで実際に描画されたルールも含めて「未到達」と誤検出する
- グループセレクタ（`.a, .b, .c { ... }`）は CSSOM 上ひとつの `selectorText` にまとまるため、素朴な実装だと「いずれか1つが一致すれば全体を到達扱いにしてしまう」（グループ内の個別デッドを見落とす）。セレクタ単位で判定したい場合はカンマで分解してから照合すること
- 実行時間の目安: 約25秒（`npm test` の通常実行 約9秒 に対して増加。jsdom 上で全テストのレンダリング結果を都度スイープするため）
...
| `stub/agentDetail.js` | `game_stats.json` / `agents/*.json` / `spectator_log.jsonl` / `frontend/src/config/agents.json`（§8.3 参照） |
...
#### A. `global profile mode` の項目（出所＝`game_stats.json` / `frontend/src/config/agents.json`）
...
5. **blurb 実装** — `frontend/src/config/agents.json` に静的フィールド追加（両モード共通・別 Issue）。→ #519 で実装済み、#628 で fetch から静的 import に変更。
