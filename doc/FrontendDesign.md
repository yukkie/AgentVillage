...
| フォントサイズ | `--fs-xs: 10px`, `--fs-sm: 11px`, `--fs-base: 12px`, `--fs-md: 13px`, `--fs-lg: 14px` | 本文・メタ帯の5段（#572）。見出し・Hero等の単発大サイズ（16px以上）はトークン化対象外（§7.4.1） |
...
#### 7.4.1 フォントサイズトークン（`--fs-*`、#562/#572）

`tokens.css` に本文・メタ帯用のフォントサイズトークンを5段で定義済み（#572）。上の「サイズの相対ルール」
（#530）が定める*実装者の日常判断*（同一行内で揃える等）を、トークンによる*構造化*で一部機械的に担保する。
両者は矛盾せず棲み分ける。

| トークン | px | 統合元の生値（#562調査時点） | 主な使用文脈 |
|---|---|---|---|
| `--fs-xs` | 10px | 9, 10, 10.5 | タイムスタンプ・turn 番号・mono 系メタ・件数・薄字ラベル |
| `--fs-sm` | 11px | 11, 11.5 | 役職/CO タグ・stat・ルール名・夜行動/投票一覧行 |
| `--fs-base` | 12px | 12, 12.5 | カード本文・フォームラベル・サイドピッカー行・セクションラベル・思考ログメタ（字数/spectator限定） |
| `--fs-md` | 13px | 13 | `body` ベースサイズ |
| `--fs-lg` | 14px | 14, 13.5, 15 | エージェント名・発言本文・見出し小・装飾的単発記号・システム行テキスト（投票/処刑等）・サイドナビ見出し（ルール・勝率ランキング）・404戻るリンク |

半端値の統合は実機（Playwright）で見比べて確定した。`13.5px`（発言本文）は近い整数値が `13px` と `14px` の
両方にあったが、名前(14px)と並べて視覚比較した結果、本文の可読性を優先して `--fs-lg`（14px）側に倒した
（13px 側に丸めると小さくなる方向で、発言本文という主要コンテンツには適さないと判断）。
