...
### 大原則: テストを書けば消える問題に、仕組みで対処しない

分類・判定手順・除外リスト・ツールを設計する前に、**「これはテストを足せば無くなる問題か」を問う**。

この原則は行カバレッジに限らず、**あらゆる到達性・網羅性の指標**に適用する（CSS ルール到達性・リンクチェック・型カバレッジなど、「対象集合のうち検証が届いた割合」を測るものすべて）。

> **検出シグナル**: 分析の結果として「**本番では使うが、テストが無いだけ**」というカテゴリが立ったら、それは発見ではなく **未実施の作業（TODO）**である。そのカテゴリに名前を付けて分類体系に組み込んだ時点で気づくこと。

**実例（#620）**: CSS ルールの到達率を計測したところ、未到達ルールが「本番で到達しうるがテストが薄い（L1）」と「本物のデッド」に分かれた。L1 を*固定された分類*として扱った結果、その存在に対処するために計測手段4案の比較・6段階の人手判定手順・分類体系・doc の1セクションを設計した。実際には **L1 は7件で、対応するテストを書けば消えるカテゴリ**だった。L1 が空になれば「未到達＝デッド」が成立し、判定はそもそも機械化できる（→ #634）。

この原則を先に通すと、下の3分類チェックに持ち込まれる Missing 行そのものが減る。
