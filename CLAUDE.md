...
- スキルファイルは `.claude/skills/`（[yukkie/claude-skills](https://github.com/yukkie/claude-skills) を git submodule として参照）で管理されている。スキルを編集したら `.claude/skills/` 内でコミット・push し、その後 AgentVillage 側で submodule の参照コミットを更新する
