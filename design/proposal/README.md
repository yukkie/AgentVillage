...
## ⚠️ デザイン変更の進め方（2026-05 以降）

このディレクトリのプロトタイプ（`prototypes/`）は **歴史的参照用** です。

**デザイン変更は `frontend/` で行ってください。**

- `frontend/src/` に React + CSS Modules で実装済みのコンポーネントがあります
- スタブデータは `frontend/stub/` にあります
- `npm run dev` で即プレビューできます
- デザイン変更 → そのまま本実装に反映される一本道になっています

`prototypes/` を編集しても `frontend/` には反映されません。二重管理を避けるため、以後はプロトタイプを編集しないでください。

**Milestone 1（モック観戦）のモックアップが完成した時点で `design/proposal/` ディレクトリ全体を削除します。**
